# MAXENT SDM ANALYSIS WITH NULL MODEL COMPARISON
...
for (mult_idx in seq_along(thin_multipliers)) {
  ...
  for (rep in 1:n_replicates) {
    ...
    real_metrics[[rep]] <- model$performance %>%
      ...
      select(replicate, model_type, multiplier, regmult, classes,
  ...
  best_regmult <- real_metrics[[1]]$regmult
  best_classes <- real_metrics[[1]]$classes
...
write_csv(final_results, paste0(output_dir, "maxent_results_complete.csv"))
...
write_csv(summary_stats, paste0(output_dir, "maxent_results_summary.csv"))
...
p <- ggplot(plot_data, aes(x = multiplier, y = value, fill = model_type)) +
  ...
  labs(
    title = "Model Performance Comparison: Real vs Null Models",
