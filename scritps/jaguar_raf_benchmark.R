# RANDOM FOREST SDM ANALYSIS WITH NULL MODEL COMPARISON
...
for (mult_idx in seq_along(thin_multipliers)) {
  ...
  for (rep in 1:n_replicates) {
    ...
    # Combine presence and background data for tune_raf
    ...
    real_metrics[[rep]] <- model$performance %>%
      ...
      select(replicate, model_type, multiplier, mtry, ntree,
  ...
  if (mult == 0) {
    ...
    best_mtry <- real_metrics[[1]]$mtry
    best_ntree <- real_metrics[[1]]$ntree
    ...
    for (rep in 1:n_replicates) {
      ...
      # Combine null presences and background data for tune_raf
...
write_csv(final_results, paste0(output_dir, "raf_results_complete.csv"))
...
write_csv(summary_stats, paste0(output_dir, "raf_results_summary.csv"))
...
p <- ggplot(plot_data, aes(x = multiplier, y = value, fill = model_type)) +
  ...
  labs(
    title = "Model Performance Comparison: Real vs Null Models (Random Forest)",
